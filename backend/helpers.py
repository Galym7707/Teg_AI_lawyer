...
def sanitize_html(html: str) -> str:
    """
    Чистим разметку и вырезаем запрещённые рекомендации вида
    «обратитесь к юристу / адвокату / специалисту».
    Также убираем лишние переносы/пустые <p>.
    """
    if not html:
        return ""
    ...
    # Унифицируем переносы/пустые строки
    h = re.sub(r'(\r\n|\r)', '\n', h)
    h = re.sub(r'\n{3,}', '\n\n', h)
    ...
    # Чуть чистим пробелы между тегами
    h = re.sub(r'>\s+<', '><', h)

    # Запрещённые формулировки → процедурная альтернатива
    banned = [
        ...
        r'за\s+юридическ(ой|ой)\s+помощ(ью|и)\s+обратит[её]сь',
    ]
    replacement = (
        "Если вопрос не удастся урегулировать досудебно, подготовьте письменную претензию/жалобу "
        "и действуйте по установленной процедуре (при необходимости — иск в суд)."
    )
    for pat in banned:
